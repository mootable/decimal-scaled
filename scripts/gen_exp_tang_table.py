...
table bakes `E_j` ONCE (oracle: flint/Arb, with rigorous interval bounds)
so the per-call cost collapses to
...
def slot_limbs_msb_first(j: int):
    ...
    if j == 0:
        # exp(0) − 1 = 0 exactly.
        return [0] * B_LIMBS
    # `ln 2` as a rigorously bounded constant; `exp(j·ln2/M)` lies in (1, 2)
    # for j in 1..M, so the fractional part is in (0, 1). Passed as a thunk
    # so the oracle can recompute it if it must escalate the precision.
    return slot_limbs(lambda: (arb.const_log2() * j / M).exp() - arb(1),
                      B, B_LIMBS, f"exp slot {j}")
...
def main():
    ...
    w("//!")
    w("//! GENERATED by `scripts/gen_exp_tang_table.py` (flint/Arb oracle —")
    ...
    w("//!")
    w("//! Any change belongs in `scripts/gen_exp_tang_table.py`, never")
    w("//! in this file — including a new `match` arm when a")
    w("//! `RoundingMode` variant is added. Re-run the generator and")
    w("//! commit both; a hand-edit here is silently reverted the next")
    w("//! time anyone regenerates.")
    w("//!")
    ...
    w("    // is the same. Round the limb count up; assert it fits the stored")
    w("    // width.")
    w("    let need_bits = (w as u64) * 3322 / 1000 + 64;")
    w("    let p_full = need_bits.div_ceil(64) as usize;")
    w("    assert!(")
    w("        p_full <= EXP_TANG_LIMBS,")
    w("        \"exp_tang: working scale {w} out of generated range ({EXP_TANG_LIMBS} limbs)\"")
    w("    );")
