...
def main():
    ...
    a("/// when the request is not served: outside the baked range")
    a("/// (`scale > MAX_SCALE` / `width > MAX_W`), or an ODD `width_limbs`")
    a("/// (see the guard below). The caller then falls back to the runtime")
    a("/// reciprocal / MgChain.")
    a("#[inline]")
