...
regression. This table bakes the pair ONCE (oracle: flint/Arb, with
rigorous interval bounds) so the
...
def slot_pair_msb_first(j: int):
    ...
    # `π` as a rigorously bounded constant; `c_j` is in (0, π/4], so
    # `sin c_j` is in (0, √2/2] and `cos c_j` in [√2/2, 1) — both in [0, 1).
    # Passed as thunks so the oracle can recompute if it must escalate.
    def c_j():
        return arb.pi() * j / (4 * M)

    return (slot_limbs(lambda: c_j().sin(), B, B_LIMBS, f"sin slot {j}"),
            slot_limbs(lambda: c_j().cos(), B, B_LIMBS, f"cos slot {j}"))
...
def main():
    ...
    w("//!")
    w("//! GENERATED by `scripts/gen_sincos_tang_table.py` (flint/Arb")
    w("//! oracle — every digit below is pinned by a rigorous interval")
    w("//! bound). Do NOT edit by hand; re-run the script and commit its")
    w("//! output.")
    w("//! This file is NOT produced at build time — `build.rs` is")
    w("//! untouched.")
    w("//!")
    w("//! Any change belongs in `scripts/gen_sincos_tang_table.py`,")
    w("//! never in this file — including a new `match` arm when a")
