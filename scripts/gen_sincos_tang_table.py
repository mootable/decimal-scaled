...
def main():
    ...
    w("    // pilot). Round the limb count up; clamp to the stored width.")
    w("    let need_bits = (w as u64) * 3322 / 1000 + 64;")
    w("    let mut p = ((need_bits + 63) / 64) as usize;")
    w("    if p < 1 {")
    w("        p = 1;")
    w("    }")
    w("    if p > SINCOS_TANG_LIMBS {")
    w("        p = SINCOS_TANG_LIMBS;")
    w("    }")
    ...
    w("            | W::from_mag_sign_u128(&[slot[k] as u128], false);")
    w("    }")
