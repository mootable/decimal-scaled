...
'''
    ...
    emit("/// floor; Ceiling always bumps (`+1`); the three half-modes all")
    emit("/// reduce to round-to-nearest = `floor + round_up`. This reproduces")
    emit("/// a correct rounding of the oracle value under every mode.")
    emit("#[inline]")
    ...
    emit("    use crate::support::rounding::RoundingMode::*;")
    emit("")
    emit("    /// The six modes derive correctly from the stored (floor,")
    emit("    /// round_up) pair: Trunc=Floor=floor, Ceiling=floor+1, and the")
    emit("    /// three half-modes coincide at floor+round_up (no ties for")
    emit("    /// irrationals). Spot-checked against `pi` at a few scales.")
    emit("    #[test]")
    ...
    emit("            for mode in [HalfToEven, Trunc, Ceiling, Floor, HalfAwayFromZero, HalfTowardZero] {")
    emit("                assert_eq!(")
