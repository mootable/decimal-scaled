...
def main():
    ...
    emit("//!")
    emit("//! Any change belongs in `scripts/gen_const_table.py`, never in")
    emit("//! this file — including a new `match` arm when a `RoundingMode`")
    emit("//! variant is added. Re-run the generator and commit both; a")
    emit("//! hand-edit here is silently reverted the next time anyone")
    emit("//! regenerates.")
    emit("//!")
    ...
    emit("//! exact tie and never zero, and the eight rounding modes derive exactly")
