...
value ONCE (oracle: flint/Arb, with rigorous interval bounds) so the
per-call cost collapses to one
multiply + one shift.
...
def slot_msb_first(j: int):
    ...
    if j == 0:
        return [0] * B_LIMBS
    # `c_j = j/M` is an exact rational, so the only rounding is Arb's own —
    # and that is carried in the ball's radius. `atan(c_j)` is in (0, π/4].
    # Passed as a thunk so the oracle can recompute it if it must escalate.
    return slot_limbs(lambda: arb(fmpq(j, M)).atan(),
                      B, B_LIMBS, f"atan slot {j}")
...
def main():
    ...
    w("//!")
    w("//! GENERATED by `scripts/gen_atan_tang_table.py` (flint/Arb oracle")
    w("//! — every digit below is pinned by a rigorous interval bound).")
    w("//! Do NOT edit by hand; re-run the script and commit its output.")
    w("//! This file is NOT produced at build time — `build.rs` is")
    w("//! untouched.")
    w("//!")
    w("//! Any change belongs in `scripts/gen_atan_tang_table.py`, never")
