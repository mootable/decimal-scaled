...
- **CHECK every algorithm's `std` requirement.** A float (`f64`) **seed** via a **std-only intrinsic** (`f64::sqrt`/`exp`/`ln`/`cbrt`/…; in `std::f64`, NOT `core`) would make the algorithm std-only. A seed only affects *convergence*, NOT the result (the self-correcting integer iteration pins the same exact value), so the **simplest fix is to cfg-swap the seed-generation step INSIDE the algorithm** — `#[cfg(feature = "std")]` fp seed else a `core` integer seed (e.g. bit-length-based). This keeps it **ONE `Algorithm` variant**, identical result both ways. (Use the policy feature-flagging strategy — separate variants — only when a *whole different algorithm* is std-only, not merely the seed.) The default `no_std` build runs the core seed and MUST be correct — verify `cargo check --no-default-features` (+ `--features alloc` if needed).
