...
- **PRESERVE THE EXISTING ROUTING — keep the researched/benched algorithm *choices* (not necessarily the same code).** The Phase-4 work is a STRUCTURAL refactor; it must NOT change *which algorithm* runs for any `(width, scale)` cell, drop a cell, or move a crossover. But the **implementation MAY be consolidated/generalised** — a cell can move to a **generic-over-`N` version of the SAME algorithm** (e.g. a generic `sqrt_mg_divide`/`div_mg` replacing the D38-specific one), as long as (a) the cell still routes to that algorithm, and (b) precision (golden) AND benched perf hold. Per-tier shims collapse to one generic algorithm when they did the same computation (the pilot's 10 `sqrt_dNN` → one generic `sqrt_newton`). Keep a cell on **bespoke code only when generalising would lose the intrinsic/perf** (Q4.1b — e.g. the pilot kept `sqrt_mg_divide` width-specific for the 256-bit `i128`/hardware path). Source of truth = the current dispatch (`impl …Policy` / `(W,SCALE)` match) + the migration doc. Golden guards precision; the final consistency pass audits **routing-parity** (new `select` arms ↔ old dispatch — same *algorithm* per cell).
