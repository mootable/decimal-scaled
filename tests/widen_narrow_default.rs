...
//! comprehensive ladder (D38 → D56 → D76 → D114 → D153 → D230 → D307
//! → D461) rather than skipping straight to the next power-of-two
//! width. Plus the per-tier `Default` impl coverage.

#![cfg(feature = "wide")]

use decimal_scaled::{D38, D56, D76, D114};

#[test]
fn d38_widen_to_d56() {
    let a = D38::<12>::from_int(7);
    let w: D56<12> = a.widen();
    let expected: D56<12> = a.into();
    assert_eq!(w, expected);
}

#[test]
fn d76_narrow_to_d56_in_range() {
    // D38 -> D56 -> D76 widens losslessly, then D76.narrow() back to
    // D56 should recover the value.
    let small: D56<12> = D38::<12>::from_int(7).into();
    let w: D76<12> = small.widen();
    let n: D56<12> = w.narrow().unwrap();
    assert_eq!(n.to_bits().to_string(), small.to_bits().to_string());
}

#[test]
fn d76_narrow_to_d56_out_of_range_errors() {
    // D76<0>::MAX is way above D56<0>::MAX
    let w = D76::<0>::MAX;
    let r: Result<D56<0>, _> = w.narrow();
    assert!(r.is_err());
}

#[test]
fn d76_widen_to_d114() {
    let a: D76<6> = D38::<6>::from_int(7).into();
    let b: D114<6> = a.widen();
...
fn d153_widen_to_d230_then_d307() {
    ...
    let a: D153<6> = D76::<6>::from_int(7).widen().widen(); // D76 -> D114 -> D153
    ...
    let r: Result<D114<0>, _> = big_153.narrow();
    assert!(r.is_err());
