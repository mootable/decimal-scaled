...
//! (`let r: D38<6> = a.reshape();`). A caller never writes limb counts — those are
//! an internal representation detail and must not appear at a call site.

use decimal_scaled::{D18, D38, RoundingMode};

#[test]
fn identity_reshape_is_a_no_op() {
    let a = D18::<2>::try_from(42i64).unwrap();
    let r: D18<2> = a.reshape();
    assert_eq!(r, a);
}
...
fn growing_scales_up_at_the_wider_width_not_the_narrow_one() {
    // This is the case that a naive "rescale then resize" gets wrong: the
    ...
    let r: D38<6> = a.reshape();                                // needs 1e24
...
fn scale_down_honours_an_explicit_rounding_mode() {
    ...
    let up: D18<2> = a.reshape_with(RoundingMode::HalfAwayFromZero);
    assert_eq!(up.to_string(), "0.01");
    let down: D18<2> = a.reshape_with(RoundingMode::Trunc);
