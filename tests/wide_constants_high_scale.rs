...
use decimal_scaled::{D256, D512, D1024, DecimalConsts};
...
/// D256<76>::pi() used to panic at the i128 rescale-up. After wiring
...
fn d256_pi_at_max_scale_does_not_panic() {
    ...
    // SCALE=50: deeper than D128 but inside D256's max of 76.
    let pi50 = D256::<50>::pi();
...
/// Cross-tier check: D256<37> and D128<37>::pi() should produce the
...
fn d256_pi_at_scale_37_matches_d128() {
    ...
    let n = D128::<37>::pi().to_bits();
    let w = D256::<37>::pi().to_bits();
    let w_as_i128 = w.to_i128_checked().expect("fits");
    let diff = (w_as_i128 - n).abs();
    assert!(diff <= 1, "D256<37>::pi {w_as_i128} vs D128<37>::pi {n}");
}
