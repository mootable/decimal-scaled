...
use decimal_scaled_golden::{
    Capabilities, CaseLoader, FnSupport, Function, Outcome, OverflowValidator, Overflow,
    ParallelTester, RoundingMode, RoundingValidator, RunOnce, Subject, TestCollector, Tester,
};
...
macro_rules! run_subjects {
    ($tester:expr, $funcs:expr, $($D:ty => ($w:expr, $s:expr, $bits:expr)),+ $(,)?) => {{
        let mut tc = TestCollector::new();
        $(
            tc.add($tester.run(
                &DsSubject::<$D> { width: $w, scale: $s, storage_bits: $bits, _p: PhantomData },
                $funcs,
            ));
        )+
        tc
