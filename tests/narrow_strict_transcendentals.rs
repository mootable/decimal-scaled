//! Coverage suite for `macros/strict_transcendentals.rs` — the D18
//! strict transcendentals that delegate to D38's `*_strict` core.
//!
//! D18 transcendentals are macro-emitted wrappers that widen `self`
//! to `D38<SCALE>`, call the D38 `*_strict` implementation, and narrow
//! back. The 0.5 ULP contract is inherited from D38 (validated by the
//! `decimal-scale-test` golden gate); the role of THIS suite is to
//! prove the macro-emitted delegators are wired up and survive the
//! narrow-tier round-trip.
//!
//! For each method we:
//! 1. Call the `*_strict` form on D18<S> at scales that fit
//!    the result, and check the bits match the truth-to-storage value
//!    (truth obtained from the same D38 method).
//! 2. The dispatcher-delegation contract (plain `*` form bit-identical
//!    to `*_strict`) moved to `decimal-scale-test`
//!    `tests/contracts/routing.rs`.
//!
//! 0.5 ULP is asserted via a `≤ 1 LSB` slack (the truth itself is
//! rounded to storage, so the slack absorbs a single-LSB transcription
//! error).
...
// when the D38 result doesn't fit the D18 storage. We deliberately
