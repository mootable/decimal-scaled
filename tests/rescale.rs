...
fn with_scale_matches_rescale() {
    // Native tier.
    let a = D38s2::from_bits(decimal_scaled::Int::<2>::try_from((150) as i128).unwrap());
    ...
    let b = D38s12::from_bits(decimal_scaled::Int::<2>::try_from((12_345_678_901_234) as i128).unwrap());
...
fn rescale_up_appends_zeros() {
    let cents = D38s2::from_bits(decimal_scaled::Int::<2>::try_from((150) as i128).unwrap());
...
#[test]
fn rescale_up_zero() {
    let z = D38s2::from_bits(decimal_scaled::Int::<2>::try_from((0) as i128).unwrap());
    let m = z.rescale::<12>();
...
fn rescale_up_overflow_panics() {
    let big = D38s12::from_bits(decimal_scaled::Int::<2>::try_from((i128::MAX) as i128).unwrap());
...
fn rescale_down_truncates_zero_remainder() {
    let micros = D38s6::from_bits(decimal_scaled::Int::<2>::try_from((1_500_000) as i128).unwrap());
...
fn rescale_down_half_to_even_rounds_to_even() {
    ...
    let micros = D38s6::from_bits(decimal_scaled::Int::<2>::try_from((1_225_000) as i128).unwrap());
    assert_eq!(
        i128::from(micros.rescale_with::<2>(RoundingMode::HalfToEven).to_bits()),
        122
    );
...
fn rescale_with_trunc_vs_floor_diverge_on_negative() {
    ...
    let micros = D38s6::from_bits(decimal_scaled::Int::<2>::try_from((-1_234_500) as i128).unwrap());
...
fn rescale_value_matches_half_to_even() {
    let src: D38s6 = D38s6::from_bits(decimal_scaled::Int::<2>::try_from((1_235_000) as i128).unwrap());
