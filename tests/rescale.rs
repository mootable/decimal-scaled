...
fn with_scale_matches_rescale() {
    ...
    assert_eq!(a.with_scale::<6>().to_bits().as_i128(), a.rescale::<6>().to_bits().as_i128());
    assert_eq!(a.with_scale::<2>().to_bits().as_i128(), a.to_bits().as_i128());
    ...
    assert_eq!(b.with_scale::<6>().to_bits().as_i128(), b.rescale::<6>().to_bits().as_i128());
}
...
fn rescale_up_appends_zeros() {
    ...
    assert_eq!(micros.to_bits().as_i128(), 1_500_000);
}
...
fn rescale_down_truncates_zero_remainder() {
    ...
    assert_eq!(cents.to_bits().as_i128(), 150);
}
...
fn rescale_down_half_to_even_rounds_to_even() {
    ...
    assert_eq!(
        micros.rescale_with::<2>(RoundingMode::HalfToEven).to_bits().as_i128(),
        124
    );
...
fn rescale_down_non_half_goes_nearest() {
    ...
    assert_eq!(micros.rescale::<2>().to_bits().as_i128(), 123);
    // 1.235001 -> 1.24 (above half)
    let micros = D38s6::from_bits(decimal_scaled::Int::<2>::from_i128(1_235_001));
    assert_eq!(micros.rescale::<2>().to_bits().as_i128(), 124);
}
...
fn rescale_with_each_mode_at_exact_half() {
    ...
    assert_eq!(micros.rescale_with::<2>(RoundingMode::Trunc).to_bits().as_i128(), 123);
    assert_eq!(micros.rescale_with::<2>(RoundingMode::Floor).to_bits().as_i128(), 123);
    assert_eq!(
        micros.rescale_with::<2>(RoundingMode::Ceiling).to_bits().as_i128(),
        124
    );
}
...
fn rescale_with_each_mode_at_exact_half_negative() {
    ...
    assert_eq!(
        micros.rescale_with::<2>(RoundingMode::Trunc).to_bits().as_i128(),
        -123
    );
    assert_eq!(
        micros.rescale_with::<2>(RoundingMode::Floor).to_bits().as_i128(),
        -124
    );
    assert_eq!(
        micros.rescale_with::<2>(RoundingMode::Ceiling).to_bits().as_i128(),
...
fn rescale_same_scale_is_bit_identity() {
    ...
    assert_eq!(same.to_bits().as_i128(), 123_456_789_012);
}
...
fn rescale_with_same_scale_is_bit_identity_for_every_mode() {
    ...
    for m in [
        ...
        assert_eq!(v.rescale_with::<12>(m).to_bits().as_i128(), 123_456_789_012, "{m:?}");
    }
}
...
fn rescale_value_matches_half_to_even() {
    let src: D38s6 = D38s6::from_bits(decimal_scaled::Int::<2>::from_i128(1_235_000));
    let dst: D38s2 = src.rescale::<2>();
    assert_eq!(dst.to_bits().as_i128(), 124);
}
