...
// ════════════════════════════════════════════════════════════════════
//
// `tests/golden/` holds the full mpmath oracle surface: 22 transcendental
// methods over 12 width tiers, at the five-point scale set per tier
// (`{0, S/4, S/2, 3S/4, S-1}`). The comparative sweep scores ALL of them
// by scanning the directory at runtime and reverse-mapping each filename
// `<stem>_d<N>_s<scale>.txt` to `(Method, Width, scale)` — so the roster
// grows automatically as golden files are added, with no hardcoded list.
