...
- **Order the arms by LEAST WORK FIRST (architectural-review Class M).** The map tells you WHICH algorithm wins per cell; the ORDER you wire the arms in decides how much work each cell does to find that out. Try the cheapest discriminator that can settle a cell and escalate only if it did not decide: tier 0 is the const key (`N`, `SCALE`) and costs nothing at run time; tier 1 is cheap operand inspection; tier 2 is derived work (magnitude assembly, `10^SCALE`, a division). `ByValue`/`ByShape` is the only verdict that stays a runtime branch, so wire every cell the map settles from `(N, SCALE)` as a `ByAlgorithm` arm ABOVE any runtime arm — `match` is first-match-wins, so a broad `ByValue` intercepts the const-decidable cells beneath it and makes each pay a branch forever. Where the map records a VALUE-dependent win region, narrow the `ByValue` to exactly that region rather than blanketing the width, and order the tests inside it cheapest-first. This is a FIDELITY question as much as a perf one: a map whose per-cell winner is right but whose arms are ordered so a runtime arm shadows them has NOT reproduced the map.
