...
- **CONST ARMS FIRST — reach a runtime arm ONLY when the consts cannot decide (owner, governing; architectural-review Class M).** *"The policy should evaluate compiled-out rules first, and then only if needed apply the non-compiled-out rules."* Because `ByValue`/`ByShape` is the only verdict that survives compilation as a branch, `select` must resolve every cell it CAN from its const key and fall through to a runtime arm ONLY over the region where the operand genuinely decides. `match` is first-match-wins, so a broad runtime arm (`(2, _) => ByValue(..)`) INTERCEPTS every scale of that width — including the ones `(N, SCALE)` already settles, which then pay a branch and lose dead-arm elimination in every monomorphisation, forever. Write the const carve-outs ABOVE the runtime arm and narrow the runtime arm to its true region. **Sanity test when you add a `ByValue`:** for each cell it covers, ask whether its body's test could be evaluated from `N` and `SCALE` alone — if yes for some sub-range, that sub-range belongs in a `ByAlgorithm` arm above it. A genuinely operand-dependent choice (magnitude against an overflow bound, sign, a near-tie) is exactly what the variant is FOR and is not a defect; the question is never whether a `ByValue` exists but whether it is REACHED for cells the consts already answer. Correctness-neutral, so no test will ever catch this.
