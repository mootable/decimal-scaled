...
- **ORDER THE ARMS BY LEAST WORK FIRST (owner, governing; architectural-review Class M).** *"The principle is the order of least work needed to be done."* Try the CHEAPEST discriminator that can settle a cell; escalate only if it did not decide. Tier 0 is the const key (`N`, `SCALE`) — it costs ZERO at run time, because `const { select::<…>() }` folds to one direct call and dead-eliminates the rest. Tier 1 is cheap operand inspection (sign, is-zero, limb length, one comparison). Tier 2 is derived work (assembling a magnitude, `10^SCALE`, `bit_length`, a division). `ByValue`/`ByShape` is the only verdict that survives as a branch, so it is always tier 1 or 2 — put every cell the consts can settle in a `ByAlgorithm` arm ABOVE it. `match` is first-match-wins, so a broad runtime arm (`(2, _) => ByValue(..)`) INTERCEPTS every scale of that width, including the ones `(N, SCALE)` already fixes, which then pay a branch and lose dead-arm elimination in every monomorphisation, forever. **The same rule applies INSIDE the closure:** order its tests cheapest-first so the expensive one runs only when the cheap one failed to decide. **Sanity test when you add a `ByValue`:** for each cell it covers, could the body's test be evaluated from `N` and `SCALE` alone — and does the body do tier-2 work before a tier-1 test that would have short-circuited it? A genuinely operand-dependent choice is exactly what the variant is FOR and is not a defect; the question is whether any cell does more work than it needed to reach the same verdict. Correctness-neutral, so no test will ever catch this.
...
Instead the kernel is generic over `N` **only** and does the wide work **directly in a limb scratch buffer**, calling the int layer's width-agnostic slice kernels (full mechanism + the three scratch forms in §5 "Scratch sizing — the `ComputeLimbs` trait"). The only trait involved binds a scratch **buffer** (`Limbs::<N>::double_buffered_u64()` and siblings, associated-type sized), not a work integer — and its bound stops at `dispatch`, never reaching the type method.

- Result: the policy file stays pure `Algorithm`/`Select`/`select`/`dispatch` — **`select` + `dispatch` do the work**, no per-tier trait/macro, no algorithm bodies, no `unreachable!()` panic arms, no `_ => self.*_strict_with(…)` escape (that's the inversion). `dispatch` carries no work-width parameter; it adds only `where Limbs<N>: ComputeLimbs` when it calls a limb-scratch root kernel.
...
**Work-width — expand in LIMBS, never name `Int<2N>` (the limb-expansion lesson):** when a decimal kernel needs a WIDER work integer than its storage (the `sqrt` radicand spans `2N` limbs, `cbrt` `4N`, `mul`/`div` `2N`), do **NOT** thread a work *type* `W = Int<2N>`. `Int<2N>` is unnameable from a generic `N` on stable (`2*N` in type position needs nightly `generic_const_exprs`), and faking it forces the non-conformant `*Policy` trait + `*_policy_tier!` per-tier macro **and** pollutes the type-method layer with an algorithm-internal width. INSTEAD compute the wider work **directly in a fixed limb scratch buffer** (sized by the `ComputeLimbs` associated-type constructors on `Limbs<N>` — see *Scratch sizing* below) and call the int layer's **width-agnostic slice kernels**: `int::algos::isqrt::isqrt_newton::isqrt_newton(&n, &mut out)` / `icbrt_newton` (roots), `int::algos::mul::mul_schoolbook::mul_schoolbook` (products, incl. building `mag·10^k` by iterative ×10), `int::policy::div_rem::dispatch` (divide), and `int::algos::support::limbs::{add_assign, sub_assign, cmp, cmp_cross, shl, shr, bit_len}`. The kernel is then generic over `N` **only** (no `W`), `dispatch` carries no work-width parameter, and the policy stays a pure `(N, SCALE)` matcher — no per-tier trait/macro, no pollution, and *more* honest about the §1a boundary than a phantom `W: BigInt`. (Done for `sqrt`/`cbrt`/`hypot` + the int roots.)

**Scratch sizing — the `ComputeLimbs` trait (in `src/int/types/compute_limbs.rs`):** a decimal root kernel gets its limb scratch from the constructors on `Limbs<N>` — `single_u64()` / `single_u128()` / `single_buffered_u64()` / `double_u64()` / `double_u128()` / `double_buffered_u64()` (the `2N + ⌈N/2⌉` sqrt/hypot/isqrt radicand) / `quad_u64()` / `quad_buffered_u64()` (the `4N + ⌈N/2⌉` cbrt/icbrt radicand) / `karatsuba_u64()` / `karatsuba_u128()` / the `digit_formatting_u8` + `bit_formatting_u8` render drains — bounding only on `where Limbs<N>: ComputeLimbs`. From a generic `W` the bound is `where W::Scratch: ComputeLimbs` (`BigInt::Scratch` is `Limbs<N>`), used as `W::Scratch::single_u128()`. The buffer **size lives in the trait's associated types** (`SingleU64`, `DoubleBufferedU64`, `KaratsubaU128`, …), i.e. in the `impl` where `N` is concrete — so it is NEVER a generic const expression in the kernel. That is the whole trick: the `where Limbs<N>: ComputeLimbs` bound is discharged for free at the concrete `N` every type method dispatches at, so it stops at `dispatch` and does **not** cascade up to the macro-generated type methods.

> **NAMING — verified in-tree 2026-08-30.** This trait was previously called `WorkScratch` (and before that `ComputeInt`), living at `int/types/work_scratch.rs` with `Int::<N>::work2()`/`work4()` and `Buf2`/`Buf4` associated types. **None of those names exist in `src/` any more** — do not write code against them. Add a new buffer the way the trait's own doc says: a literal in each of the three build-form impls, an associated type per element, and the `Limb` forwarder. (Put a `generic_const_exprs` length directly in a kernel and you get a `where [(); …]:` bound that *does* cascade through every caller — the trap this avoids.) Add the bound to BOTH the kernel and its policy `dispatch`; the type method needs nothing. Three feature-selected forms, numerically identical (only *who pays for the slack* differs):
  - **default** (`exact-scratch`, **a default feature** — `std` requires it): one `impl ComputeLimbs for Limbs<$n>` per concrete storage width via a macro, each buffer sized exactly from the literal `n` — stable, smallest frames, no const generics. The owner is against per-width impls in general but **approved them here, gated behind the feature**.
  - **no `exact-scratch`** (`--no-default-features`): ONE blanket `impl<const N: usize> ComputeLimbs for Limbs<N>` sized to the build-max width (`MAX_WORK_N` = the widest *enabled* tier's limb count, 2/16/32/64) and applied uniformly. The prior behaviour, kept as the no-per-impl fallback.
  - **nightly**: ONE blanket impl sized per-`N` under `generic_const_exprs`; the const-expr + its `where [(); …]:` obligation stay **confined to that impl block** (the associated-type indirection is what confines it), so no cascade even here.
  All three live in `src/int/types/compute_limbs.rs` and are numerically identical; only *who pays for the slack* differs. The int slice roots (`isqrt_newton`/`icbrt_newton`) take runtime slices with no const `N`, so they keep a build-max sizing. **Run golden with `exact-scratch` on (it's default) as part of the default-build gate** — a too-small per-`N` buffer is release-mode UB that only the golden will catch. A too-small scratch elsewhere is the same hazard: validate any reduced band with a golden/lib run for that feature set.
...
- **Quick validation (agents, during development):** a FILTERED debug slice of the golden gate — `GOLDEN_WIDTHS=<tiers> GOLDEN_FUNCS=<fns> cargo test -p decimal-scale-test --test golden golden_default -- --ignored --nocapture` (seconds at narrow widths; add `--features wide,x-wide,xx-wide` for wide tiers). Run it freely while iterating; keep slices small — the full surface is the coordinator/CI's striped run.
...
- **Regression capture — pin the INPUT VALUE that exposed the bug.** Whenever you find a wrong/inaccurate result, the fix is NOT complete until the **specific input value** that produced it (the exact operand(s), at that width + scale) is added to the **holistic golden suite** so it can never silently regress. Add THAT input — with its correctly-rounded expected value taken from the EXTERNAL oracle, never from the now-fixed code — to the function's `decimal-scaled-golden/lead/<fn>.pb` input file with a `// regression:` why-comment (the oracle pipeline in `decimal-scaled-golden/oracle/` regenerates the `.au` rows, validators included), or, if the op isn't oracle-backed, to a regression test in `decimal-scale-test/tests/regressions/`. The discovery of a bug is the trigger to widen the suite with that input, every time. (The `hypot` lesson: the +8–13 ULP error survived precisely because no holistic test pinned the inputs that exposed it.)
- **External oracle, never self-validation.** Expected values come from an EXTERNAL high-precision source — the multi-oracle golden set `decimal-scaled-golden/golden/<fn>.au` (generated from `decimal-scaled-golden/lead/<fn>.pb` by the oracle pipeline in `decimal-scaled-golden/oracle/`), or for a one-off, a value computed offline with mpmath / Python `decimal` and baked in **with a source-citing comment**. NEVER assert against the crate's own output (circular).
...
- **All eight RoundingModes, every width.** The harness folds the oracle's `(floor_raw, cls)` to the correctly-rounded integer for ANY mode; assert across all eight modes and every supported width — the six original ones plus `AwayFromZero` (GDA `round-up`) and `ZeroFiveUp` (GDA `round-05up`). Split the test per width so local iteration stays fast. NOTE: a hardcoded `[RoundingMode; 6]` still COMPILES after a variant is added — it just covers less, silently. Prefer a shared list over a per-file array, and when the enum grows, sweep for stale lengths.
...
- **New function → add inputs to `decimal-scaled-golden/lead/<fn>.pb`** and regenerate via the oracle pipeline (`decimal-scaled-golden/oracle/generate.py`); the gate derives every `(width, scale)` cell automatically.
- **While iterating, run a filtered slice** of the golden gate (above) — never the full surface. The coordinator/CI runs the full striped gate before merge/push.
