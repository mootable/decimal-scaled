...
**Scratch sizing — `work_scratch(mult)` (in `int/algos/support/limbs.rs`):** the buffer size is `int::algos::support::limbs::work_scratch(mult)` = `mult·MAX_WORK_N + ceil(MAX_WORK_N/2)` (the `mult·N` work width + a `0.5·N` carry-limb margin). `mult = 2` for the **2N-family** (`sqrt`/`hypot`/`isqrt_newton`, radicand ≤ 2N), `mult = 4` for the **4N-family** (`cbrt`/`icbrt_newton`, radicand ≤ 4N). `MAX_WORK_N` is ONE feature-gated const = the widest *enabled* tier's limb count (2 / 16 / 32 / 64 for default / `wide` / `x-wide` / `xx-wide`), so narrow builds get a small stack frame (default `work_scratch(2) = 5`) while `xx-wide` reproduces the proven `work_scratch(4) = 288`. **ONE const + one `const fn`, no per-`Int<N>` impls** (the owner is against per-width impls). Validate any reduced band with a golden/lib run for that feature set (a too-small scratch is release-mode UB). A future `nightly`/`generic_const_exprs` path can size to the call's *actual* `N` instead of the build max.
