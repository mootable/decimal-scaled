...
- Operands must share the same `SCALE` (and width). Cross-scale needs `value.quantize::<TARGET>()`, or the explicit-target `mul_of` (see below).
...
- `quantize<TARGET>` / `widen_n<M>` - need a const-generic method parameter,
