//! `num-traits` 0.2 trait implementations for [`I128`].
//!
//! Allows generic numeric code (nalgebra, ndarray, statrs, and other
//! crates that accept "any number type") to use `I128<SCALE>` as a
...
//!
//! Both traits delegate to the inherent [`I128::checked_mul`] and
//! [`I128::checked_div`] methods. The trait and inherent paths are
...
//
// CheckedMul and CheckedDiv delegate to the inherent I128::checked_mul and
// I128::checked_div methods. The trait and inherent paths are bit-identical.
...
mod tests {
    ...
    fn to_primitive_i64_out_of_range_returns_none() {
        // I128::MAX integer part ~= 1.7e26, which exceeds i64::MAX.
    ...
    fn to_primitive_smaller_int_types_via_default_impl() {
        ...
        let big = I128s12::from_bits(40_000_000_000_000_000); // 40_000
        assert_eq!(<I128s12 as ToPrimitive>::to_i8(&big), None);
    ...
    /// For 256 deterministic pairs, `<I128 as CheckedMul>::checked_mul`
    /// must equal `I128::checked_mul` (the inherent method).
    #[test]
    ...
    /// For 256 deterministic pairs, `<I128 as CheckedDiv>::checked_div`
    /// must equal `I128::checked_div` (the inherent method).
    #[test]
