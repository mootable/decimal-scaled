...
//! # Wire format
//!
//! `I128<SCALE>` chooses its wire encoding based on the serializer's
...
//!   `I128s12::ONE` (storage `1_000_000_000_000`) serialises as the
...
///
/// Use this module when you want to control serialisation of a `I128`
...
pub mod decimal_serde {
    ...
    /// Serialise `v` using the `I128` wire format.
    ///
    ...
    /// Deserialise a `I128` using the wire format.
    ///
    ...
    pub fn deserialize<'de, const SCALE: u32, D: Deserializer<'de>>(
        d: D,
    ) -> Result<I128<SCALE>, D::Error> {
        I128::<SCALE>::deserialize(d)
    }
    ...
    impl<'de, const SCALE: u32> Visitor<'de> for DecimalVisitor<SCALE> {
        ...
        fn visit_bytes<E: serde::de::Error>(self, v: &[u8]) -> Result<Self::Value, E> {
            ...
            Ok(I128::<SCALE>::from_bits(i128::from_le_bytes(arr)))
        }
        ...
        fn visit_i8<E: serde::de::Error>(self, v: i8) -> Result<Self::Value, E> {
            Ok(I128::<SCALE>::from_bits(v as i128))
        }
        ...
        fn visit_u128<E: serde::de::Error>(self, v: u128) -> Result<Self::Value, E> {
            ...
            i128::try_from(v).map(I128::<SCALE>::from_bits).map_err(|_| {
                serde::de::Error::custom(
...
mod tests {
    ...
    /// directly produces a `I128` whose raw bits are `-5`.
    #[test]
    fn visitor_accepts_i64_as_storage() {
        let visitor = decimal_serde::DecimalVisitor::<12>(PhantomData);
        let v: I128s12 = <_ as Visitor>::visit_i64::<DeError>(visitor, -5).unwrap();
    ...
    fn visitor_rejects_u128_above_i128_max() {
        let visitor = decimal_serde::DecimalVisitor::<12>(PhantomData);
        let res: Result<I128s12, _> = <_ as Visitor>::visit_u128::<DeError>(
            visitor,
            (i128::MAX as u128) + 1,
        );
    ...
    fn json_one_round_trips() {
        let json = serde_json::to_string(&I128s12::ONE).unwrap();
        let back: I128s12 = serde_json::from_str(&json).unwrap();
        assert_eq!(back, I128s12::ONE);
    }
    ...
    /// `I128::MAX` and `I128::MIN` round-trip exactly through the
    /// JSON-string wire format.
    #[test]
    fn json_max_round_trips() {
        let json = serde_json::to_string(&I128s12::MAX).unwrap();
        let back: I128s12 = serde_json::from_str(&json).unwrap();
        assert_eq!(back, I128s12::MAX);
    }

    #[test]
    fn json_min_round_trips() {
        let json = serde_json::to_string(&I128s12::MIN).unwrap();
        let back: I128s12 = serde_json::from_str(&json).unwrap();
        assert_eq!(back, I128s12::MIN);
    }
    ...
    fn json_rejects_decimal_point_string() {
        let res: Result<I128s12, _> = serde_json::from_str("\"1.5\"");
        assert!(res.is_err(), "expected reject; got Ok({:?})", res);
    }

    #[test]
    fn json_rejects_scientific_notation_string() {
        let res: Result<I128s12, _> = serde_json::from_str("\"1e6\"");
        assert!(res.is_err(), "expected reject; got Ok({:?})", res);
    }

    #[test]
    fn json_rejects_not_a_number_string() {
        let res: Result<I128s12, _> = serde_json::from_str("\"not-a-number\"");
        assert!(res.is_err(), "expected reject; got Ok({:?})", res);
    }

    #[test]
    fn json_rejects_empty_string() {
        let res: Result<I128s12, _> = serde_json::from_str("\"\"");
        assert!(res.is_err(), "expected reject; got Ok({:?})", res);
    }
    ...
    fn json_rejects_leading_whitespace_string() {
        ...
        let res: Result<I128s12, _> = serde_json::from_str("\"  42\"");
        assert!(res.is_err(), "expected reject; got Ok({:?})", res);
    }
    ...
    fn json_accepts_bare_integer_number_as_storage() {
        let back: I128s12 = serde_json::from_str("42").unwrap();
    ...
    fn postcard_one_round_trips() {
        let bytes: alloc::vec::Vec<u8> = postcard::to_allocvec(&I128s12::ONE).unwrap();
        // Verify the raw 16 LE bytes appear somewhere in the postcard
        // output (postcard may prepend a varint length prefix).
        let raw = I128s12::ONE.to_bits().to_le_bytes();
        assert!(bytes.windows(16).any(|w| w == raw));
        let back: I128s12 = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(back, I128s12::ONE);
    }

    #[test]
    fn postcard_zero_round_trips() {
        let bytes: alloc::vec::Vec<u8> = postcard::to_allocvec(&I128s12::ZERO).unwrap();
        let back: I128s12 = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(back, I128s12::ZERO);
    }
    ...
    fn postcard_max_round_trips() {
        let bytes: alloc::vec::Vec<u8> = postcard::to_allocvec(&I128s12::MAX).unwrap();
        let back: I128s12 = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(back, I128s12::MAX);
    }
    ...
    fn postcard_min_round_trips() {
        let bytes: alloc::vec::Vec<u8> = postcard::to_allocvec(&I128s12::MIN).unwrap();
        let back: I128s12 = postcard::from_bytes(&bytes).unwrap();
        assert_eq!(back, I128s12::MIN);
    }
    ...
    fn postcard_byte_order_matches_le() {
        let v = I128s12::from_bits(0x0123_4567_89AB_CDEF_FEDC_BA98_7654_3210_i128);
        let bytes: alloc::vec::Vec<u8> = postcard::to_allocvec(&v).unwrap();
    ...
    fn cross_scale_wire_is_storage_only() {
        ...
        let v12 = I128::<12>::from_bits(raw);
        let v6 = I128::<6>::from_bits(raw);
