...
//!
//! # The two-variant matrix
//!
//! Each transcendental ships two entry points so a single name
//! covers either rounding-mode choice:
//!
    ...
    ($Type:ident, $Storage:ty, $Work:ty, $Wexp:ty, $AgmWork:ty, $core:ident, $max_scale:literal, $table_mode:ident,
     $n_limbs:literal, $ln_tang_cap:literal, $exp_tang_m:literal) => {
        ...
        pub(crate) mod $core {
            ...
            /// `10^working_digits` instead of the const `GUARD`. Used
            /// wherever the guard width is a runtime value rather than
            /// the tier's constant — the Ziv escalation rungs.
