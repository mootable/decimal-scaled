...
macro_rules! decl_pow10_cached {
    ...
    (no_const_table, $max_scale:literal) => {
        /// Stateless companion to [`pow10`] keyed on `w`. This tier's
        /// max scale puts the const-table build past the stable-rust
        /// const-eval step budget (the `with_const_table` arm's
        /// `static POW10_TABLE` will not compile here), so there is no
        /// compile-time table to index — `pow10_cached` recomputes
        /// `10^w` on the stack each call.
        ///
        /// FLAGGED (Phase 9, build.rs follow-up): the value is
        /// value-independent and would bake cleanly as a `static`
        /// generated by a `build.rs` codegen step (sidestepping the
        /// const-eval step budget that blocks the in-crate `const fn`
        /// builder for D924 / D1232). Not added this pass per the task
        /// scope; until then this tier recomputes (Rule-2-correct,
        /// heap-free) rather than carrying a runtime cache.
        #[inline]
        pub(crate) fn pow10_cached(w: u32) -> W {
            cached(w, pow10)
        }
    ...
    ($Type:ident, $Storage:ty, $Work:ty, $Wexp:ty, $core:ident, $max_scale:literal, $table_mode:ident,
     $n_limbs:literal, $ln_tang_cap:literal, $exp_tang_m:literal) => {
        ...
        pub(crate) mod $core {
            ...
            // result for a given `w` is identical for every call — so the
            // Rule-2-correct shape is a stateless recompute (no
            // thread-local memo, no heap). `cached` is the single seam the
            // `pi` / `ln2` / `ln10` / `pow10_cached`-fallback helpers route
            // through; it now recomputes directly. `pow10` for the
            // strict-path `w` range is served by the compile-time
            // `static POW10_TABLE` (see `decl_pow10_cached!`), so the only
            // recompute that survives is the out-of-range `_approx` tail
            // and the series constants, where the table cannot reach.
            #[inline]
            fn cached<F>(w: u32, compute: F) -> W
            where
                F: FnOnce(u32) -> W,
            {
                compute(w)
            ...
            //
            // The Tang ln / exp / sincos kernels index a table of
            // transcendental constants `T(w)[idx]` that is value-
            // independent: identical for every call at the same
            // `(w, M)`. The pre-Phase-9 shape memoised the whole table in
            // a `thread_local!` `Vec` (state + heap, both forbidden by
            // the Constitution's no-state / no-heap rules).
            //
            // Each kernel call needs exactly ONE slot, so the heap-free,
            // state-free replacement computes that single slot directly —
            // no `Vec`, no cache. The entry builders below are the body
            // of the former per-table loops specialised to one index.
            //
            // FLAGGED (Phase 9, build.rs follow-up): the full tables ARE
            // bakeable as compile-time `static` rodata, but only via a
            // `build.rs` codegen step — the per-slot builders call the
            // runtime `*_fixed` BigInt kernels (`ln_fixed` / `exp_fixed` /
            // `sin_cos_fixed`), which are NOT `const fn` (they run
            // `BigInt` divides), so an in-crate `const fn` builder cannot
            // produce them. Baking them (to recover the lookup speed the
            // cache used to provide) is deferred; until then every call
            // recomputes its slot on the stack (Rule-2-correct, heap-free,
            // slower — the expected, accepted Phase-9 regression).
