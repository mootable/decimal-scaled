//! Conversions between [`D128`] and primitive numeric types.
//!
//! # Naming convention
//!
...
//!   multiplier in `f64`; f64's 53-bit mantissa cannot represent every `D128`
//!   value exactly.
...
//!   `D128::MAX`; `from_f64_lossy(f64::NAN)` returns `D128::ZERO`.
...
impl<const SCALE: u32> From<i8> for D128<SCALE> {
...
impl<const SCALE: u32> From<i16> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
...
impl<const SCALE: u32> From<i32> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
...
impl<const SCALE: u32> From<i64> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
...
impl<const SCALE: u32> From<u8> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
...
impl<const SCALE: u32> From<u16> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
    /// Lossless for all `SCALE < 33`.
    ///
...
impl<const SCALE: u32> From<u32> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
    /// Lossless for all `SCALE < 28`.
    ///
...
impl<const SCALE: u32> From<u64> for D128<SCALE> {
    /// Converts `value` by scaling it to `value * 10^SCALE`.
    ///
...
impl<const SCALE: u32> TryFrom<f32> for D128<SCALE> {
    ...
    /// for finite values whose magnitude exceeds `D128::MAX` after scaling.
    ///
    /// # Precision
    ///
...
// ──────────────────────────────────────────────────────────────────────
// D128 inherent conversion methods
// ──────────────────────────────────────────────────────────────────────

impl<const SCALE: u32> D128<SCALE> {
    /// Constructs a `D128` from an `i64` integer value.
    ///
    /// Named constructor that wraps `From<i64>`. Prefer this over
    /// `D128::from(value)` when the intent of converting from an integer
    ...
    /// Constructs a `D128` from an `i32` integer value.
    ///
    ...
    /// below the `D128` maximum of ~1.7e26.
    ///
    /// # Precision
    ///
    /// Lossy: involves f32 or f64 at some point; result may lose precision.
...
mod tests {
    ...
    /// Sanity: round-trip `D128::from(int).to_int_lossy() == int as i64`
