//! Tang-style table-driven `ln_strict` kernel for `D307<SCALE>` with
//! `SCALE ∈ 285..=295` — the deep-storage band approaching the MAX
//! end of D307 (`MAX_SCALE = 306`).
//!
//! Sibling to the D307 mid-band Tang ln at
//! [`crate::algos::ln::lookup_d307_s140_160_tang`]. See Tang 1990,
...
//! converges in `log₁₀(10⁻³⁰⁵) / log₁₀(1.5·10⁻⁵) ≈ 63 pair-terms` at
//! `w = SCALE + 10 = 295..305`. Compare to the non-table wide-kernel
//! path which runs ~95 artanh muls plus several wide `sqrt_fixed`
//! calls for Brent's argument-halving — at Int4096 working width each
//! sqrt is a full-width divmod inside Newton, so the table-win is the
//! elimination of all wide sqrts plus the bulk of the artanh series.
//!
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~63 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 32 LSB-of-w. At `w = SCALE + 10`, that's
//!   `32·10⁻¹⁰` in storage units — many orders of magnitude below half
//!   a storage ULP for any `SCALE ≤ 295`.
//! - `M = 128` matches the D307 mid-band sibling slot. Per-thread
//!   memory cost: `(M+1) · sizeof(W) = 129·512 B = ~66 KB` for D307's
//!   Int4096 working integer — larger than L1d (32 KB) but comfortably
//!   inside L2 (256 KB-1 MB on modern x86).
...
/// Narrow guard for the Tang-style ln slot at SCALE 285..=295. See
...
/// Tang-style `ln(x)` strict kernel for `D307<SCALE>` with
/// `SCALE ∈ 285..=295`. Panics if `raw <= 0`.
#[inline]
#[must_use]
pub(crate) fn ln_strict<const SCALE: u32>(raw: Int1024, mode: RoundingMode) -> Int1024 {
    if raw <= Int1024::ZERO {
        panic!("D307::ln: argument must be positive");
    }
