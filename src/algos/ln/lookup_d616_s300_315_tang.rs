...
//! `SCALE = 308` (half of `MAX_SCALE = 615`).
//!
//! Sibling to the D153 mid-storage Tang ln at
...
//! converges in `log₁₀(10⁻³¹⁸) / log₁₀(1.5·10⁻⁵) ≈ 66 pair-terms` at
//! `w = SCALE + 10 ≤ 325`. Compare to the wide-tier macro `ln_fixed`
//! which runs Brent's argument-halving (4 wide `sqrt_fixed`s on a
//! 1024-byte working integer) plus the long artanh on `|m - 1| < 1`
//! — replacing the 4 wide sqrts and shortening the artanh tail is
//! the source of the speedup at D616's working width.
//!
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~70 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≤ ~36 LSB-of-w. At `w = SCALE + 10`, that's
//!   `36·10⁻¹⁰` in storage units — many orders of magnitude below
//!   half a storage ULP for any `SCALE ≤ 315`.
//! - `M = 128` matches the D57 / D115 / D153 mid-storage Tang slots.
//!   Per-thread memory cost: `(M + 1) · sizeof(W) = 129 · 1024 B
//!   ≈ 132 KB` for D616's Int8192 working integer. Larger than L1d
//!   (32 KB typical) but well inside L2 (256 KB-1 MB on modern x86).
...
/// Narrow guard for the Tang-style ln slot at SCALE 300..=315. See
