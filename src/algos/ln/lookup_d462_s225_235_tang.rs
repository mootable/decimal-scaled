...
//! converges in ~`log₁₀(10⁻ᵂ) / log₁₀(1.5·10⁻⁵) ≈ w/5` pair-terms.
//! At `w = SCALE + 10 = 235..245` that's ~47-49 pair-terms — still a
//! large win over the canonical `wide_kernel::ln_strict_d462` whose
//! Brent argument-halving runs 4 wide `sqrt_fixed` calls against the
//! `Int3072` working integer plus an artanh series at full `SCALE +
//! GUARD = 30` width.
//!
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~50 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 30 LSB-of-w. At `w = SCALE + 10`, that's
//!   `30·10⁻¹⁰` in storage units — over 200 orders of magnitude below
//!   half a storage ULP for `SCALE ≤ 235`.
//! - `M = 128` matches the D153 / D115 / D57 narrow-tier Tang slots.
//!   Per-thread memory cost: `(M + 1) · sizeof(W) = 129 · 192 B = ~25
//!   KB` for D462's Int1536 working integer — sits in L1d on every
//!   modern x86 (32 KB typical).
...
/// Narrow guard for the Tang-style ln slot at SCALE 225..=235.
/// Matches the GUARD_NARROW recommended by the D616 sibling probe.
...
/// Tang-style `ln(x)` strict kernel for `D462<SCALE>` with
/// `SCALE ∈ 225..=235`. Panics if `raw <= 0`.
#[inline]
#[must_use]
pub(crate) fn ln_strict<const SCALE: u32>(raw: Int1536, mode: RoundingMode) -> Int1536 {
    if raw <= Int1536::ZERO {
        panic!("D462::ln: argument must be positive");
    }
