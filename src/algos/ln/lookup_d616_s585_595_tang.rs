//! Tang-style table-driven `ln_strict` kernel for `D616<SCALE>` with
//! `SCALE ∈ 585..=595` — the deep-storage band approaching the MAX
//! end of D616 (`MAX_SCALE = 615`).
//!
//! Sibling to the D616 mid-band Tang ln at
...
//! converges in `log₁₀(10⁻⁶⁰⁵) / log₁₀(1.5·10⁻⁵) ≈ 125 pair-terms` at
//! `w = SCALE + 10 = 595..605`. Compare to the non-table wide-kernel
//! path which runs ~190 artanh muls plus several wide `sqrt_fixed`
//! calls for Brent's argument-halving — at Int8192 working width each
...
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~125 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 63 LSB-of-w. At `w = SCALE + 10`, that's
//!   `63·10⁻¹⁰` in storage units — many orders of magnitude below half
//!   a storage ULP for any `SCALE ≤ 595`.
//! - `M = 128` matches the D616 mid-band sibling slot. Per-thread
//!   memory cost: `(M+1) · sizeof(W) = 129·1024 B = ~132 KB` for
//!   D616's Int8192 working integer. Larger than L1d (32 KB typical)
//!   but well inside L2 (256 KB-1 MB on modern x86).
...
/// Tang-style `ln(x)` strict kernel for `D616<SCALE>` with
