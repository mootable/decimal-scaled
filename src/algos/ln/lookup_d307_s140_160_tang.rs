...
//! point. `MAX_SCALE = 306` for D307.
//!
//! Sibling to the D153 narrow-tier Tang ln at
//! [`crate::algos::ln::lookup_d153_s70_82_tang`]. See Tang 1990,
...
//! converges in `log₁₀(10⁻¹⁵⁸) / log₁₀(1.5·10⁻⁵) ≈ 33 pair-terms` at
//! `w = SCALE + 8 = 148..168`. Compare to the non-table wide-kernel
//! path which runs ~50 artanh muls plus several wide `sqrt_fixed`
//! calls for Brent's argument-halving — at Int1024 working width each
...
//! ## Tuning
//!
//! - `GUARD_NARROW = 8` — error budget: ~33 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 17 LSB-of-w. At `w = SCALE + 8`, that's
//!   `17·10⁻⁸` in storage units — many orders of magnitude below half
//!   a storage ULP for any `SCALE ≤ 160`.
//! - `M = 128` matches the D153 sibling slot. Per-thread memory
//!   cost: `(M+1) · sizeof(W) = 129·128 B = ~16.5 KB` for D307's
//!   Int1024 working integer — comfortable L1d residence on modern
//!   x86 (32 KB L1d, 256 KB-1 MB L2).
...
/// Narrow guard for the Tang-style ln slot at SCALE 140..=160. See
