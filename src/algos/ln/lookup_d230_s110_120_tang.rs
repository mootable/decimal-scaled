//! Tang-style table-driven `ln_strict` kernel for `D230<SCALE>` with
//! `SCALE ∈ 110..=120` — the mid-storage popular band centred on
//! `SCALE = 115` (half of `MAX_SCALE = 229`).
//!
//! Sibling to the D153 narrow-tier Tang ln at
//! [`crate::algos::ln::lookup_d153_s70_82_tang`] and the D462 sibling
//! at [`crate::algos::ln::lookup_d462_s225_235_tang`]. See Tang 1990,
...
//! converges in `log₁₀(10⁻¹³⁰) / log₁₀(1.5·10⁻⁵) ≈ 27 pair-terms` at
//! `w = SCALE + 10 = 120..130`. Compare to the non-table wide-kernel
//! path which runs ~40 artanh muls plus several wide `sqrt_fixed`
//! calls for Brent's argument-halving — at Int3072 working width each
//! sqrt is a full-width divmod inside Newton, so the table-win is the
//! elimination of all wide sqrts plus a chunk of the artanh series.
//!
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~27 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 14 LSB-of-w. At `w = SCALE + 10`, that's
//!   `14·10⁻¹⁰` in storage units — many orders of magnitude below half
//!   a storage ULP for any `SCALE ≤ 120`.
//! - `M = 128` matches the D153 / D462 sibling slots. Per-thread
//!   memory cost: `(M+1) · sizeof(W) = 129·384 B = ~49.5 KB` for
//!   D230's Int3072 working integer — larger than typical L1d (32 KB)
//!   but comfortably inside L2 (256 KB-1 MB on modern x86).

#![cfg(any(feature = "d230", feature = "wide"))]

use crate::int::types::Int;
use crate::support::rounding::RoundingMode;
use crate::types::widths::wide_trig_d230 as core;

/// Narrow guard for the Tang-style ln slot at SCALE 110..=120. See
...
/// Tang-style `ln(x)` strict kernel for `D230<SCALE>` with
/// `SCALE ∈ 110..=120`. Panics if `raw <= 0`.
#[inline]
#[must_use]
pub(crate) fn ln_strict<const SCALE: u32>(raw: Int<12>, mode: RoundingMode) -> Int<12> {
    if raw <= Int::<12>::ZERO {
        panic!("D230::ln: argument must be positive");
    }
