...
use crate::algos::ln::ln_series_2limb::{
    LN_GUARD, fixed_from_int256, ln_with_core, wide_ln2,
};
...
/// widest working scale (`SCALE 38 + LN_GUARD 10 = 48`) that is
/// `256 + 160 = 416` bits, comfortably inside `Int<8>`'s 512.
/// [`WORK_FITS`] turns that reasoning into a build error rather than a
/// trusted comment.
type Work = Int<8>;
...
/// `SCALE + LN_GUARD`, const-foldable at every call, so a guard or scale
/// change that would overflow `Work` must fail the BUILD. The alternative
/// — discovering it as a wrong answer or a panic in release — is the
/// failure mode the build-max sizing bug already cost this campaign once.
...
/// scale is `SCALE + LN_GUARD`, known at compile time, so "this build's
...
pub(crate) fn ln<const SCALE: u32>(raw: Int<2>, mode: RoundingMode) -> Option<Int<2>> {
    const {
        assert!(
            ln_table_fits(SCALE + LN_GUARD),
            "ln Tang: this build's ln_tang_table is too narrow for SCALE + LN_GUARD"
        );
        assert!(
            work_fits(SCALE + LN_GUARD),
            "ln Tang: slot_hi * 10^w overflows the Work integer at SCALE + LN_GUARD"
        );
    }
...
mod tests {
    ...
    fn node_snap_is_exact_on_a_table_node() {
        const W: u32 = 29; // SCALE 19 + LN_GUARD 10
        let one = Fixed {
            negative: false,
            mag: Fixed::pow10(W),
        };
