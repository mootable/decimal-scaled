...
//! `SCALE ∈ 895..=905` — the deep-storage band approaching the MAX
//! end of D924 (`MAX_SCALE = 923`).
//!
//! Sibling to the D924 mid-band Tang ln at
//! [`crate::algos::ln::lookup_d924_s455_465_tang`]. See Tang 1990,
...
//! converges in `log₁₀(10⁻⁹¹⁵) / log₁₀(1.5·10⁻⁵) ≈ 190 pair-terms` at
//! `w = SCALE + 10 = 905..915`. Compare to the non-table wide-kernel
//! path which runs ~290 artanh muls plus several wide `sqrt_fixed`
//! calls for Brent's argument-halving — at Int12288 working width
//! each sqrt is a full-width divmod inside Newton, so the table-win
//! is the elimination of all wide sqrts plus a chunk of the series.
//!
//! ## Tuning
//!
//! - `GUARD_NARROW = 10` — error budget: ~190 muls × 0.5 LSB-of-w +
//!   table-entry rounding ≈ 95 LSB-of-w. At `w = SCALE + 10`, that's
//!   `95·10⁻¹⁰` in storage units — many orders of magnitude below half
//!   a storage ULP for any `SCALE ≤ 905`.
//! - `M = 128` matches the D924 mid-band sibling slot. Per-thread
//!   memory cost: `(M+1) · sizeof(W) = 129·1536 B = ~198 KB` for
//!   D924's Int12288 working integer. Larger than L1d (32 KB typical)
//!   and possibly larger than L2 (256 KB-1 MB on modern x86) but
//!   amortised — the table is built once per thread per w.
...
/// Narrow guard for the Tang-style ln slot at SCALE 895..=905. See
