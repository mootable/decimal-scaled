...
// Per-scale, oracle-sourced, width-deduplicated wide transcendental
...
// (mpmath oracle), committed, NOT built at build time. Only the wide
// tiers request these constants, so the (large) table is gated behind
// `_wide-support` — a narrow-only build carries none of it.
