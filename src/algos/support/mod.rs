...
// `ln` tables below, plus the zero-sized `LnTangTable` selector types that
// name which of them a kernel reads. Always compiled: the narrow tiers
// (D18/D38) exist in every build and now reach the Tang `ln` path too.
// Hand-written (it is code, not oracle data); the tables it reads are
// generated.
pub(crate) mod ln_tang_slot;

// Baked binary Tang `ln(1 + i/M)` lookup table (`M = 128`), full
// `B = 7168`-bit slots. Reconstruction lives in `ln_tang_slot`; this
// module is the DATA plus the wrapper naming its slot. Generated by
// `scripts/gen_ln_tang_table.py` from a flint/Arb oracle — each slot is
// `floor(value · 2^B + 1/2)` resolved by `unique_fmpz`, which yields a
// value ONLY when the enclosing interval determines every retained bit —
// committed, NOT built at build time (`build.rs` untouched).
// Gated to `_wide-support` for its SIZE (~115 KB): the wide tiers
// (D57..D1232) need the full precision, a narrow-only build must not
// carry it. Narrow tiers read the prefix table below instead.
#[cfg(feature = "_wide-support")]
pub(crate) mod ln_tang_table;

// The narrow (`D18`/`D38`) Tang `ln` table: the top 8 limbs of each slot
// above, ~8 KB, emitted by the SAME generator run so it is the wide
// table's high-limb prefix by construction. Always compiled — the narrow
// tiers exist in every build. See `ln_tang_slot` for why there are two
// tables but only one reconstruction body.
pub(crate) mod ln_tang_table_narrow;
...
// `scripts/gen_sincos_tang_table.py` from a flint/Arb oracle — each slot
// is `floor(value · 2^B + 1/2)` resolved by `unique_fmpz`, so every
// retained bit is pinned by a rigorous interval bound — committed, NOT
...
// convert time. Generated by `scripts/gen_exp_tang_table.py` from a
// flint/Arb oracle — each slot is `floor(value · 2^B + 1/2)` resolved by
// `unique_fmpz`, so every retained bit is pinned by a rigorous interval
// bound — committed, NOT built at build time (`build.rs` untouched).
...
// in the D57 atan Tang kernel (`algos::trig::atan_tang_3limb`),
// which the samply probe showed accounted for ~74% of that kernel's
// time. Generated by `scripts/gen_atan_tang_table.py` from a flint/Arb
// oracle — each slot is `floor(value · 2^B + 1/2)` resolved by
// `unique_fmpz`, so every retained bit is pinned by a rigorous interval
// bound — committed, NOT built at build time (`build.rs` untouched). Gated to
