...
//! value. For example, `D128s12::ONE` (storage `10^12`) prints in lower-hex
//! as `e8d4a51000`.
//!
//! # FromStr
//!
...
impl<const SCALE: u32> fmt::LowerExp for D128<SCALE> {
    ...
    ///
    /// let sub = D128s12::from_bits(1_500_000_000);
...
mod tests {
    use super::*;
    use crate::core_type::{D128s12, D128};
    #[cfg(feature = "alloc")]
    use alloc::format;
    #[cfg(feature = "alloc")]
    ...
    fn display_zero_renders() {
        assert_eq!(D128s12::ZERO.to_string(), "0.000000000000");
    }
    ...
    fn display_one_renders() {
        assert_eq!(D128s12::ONE.to_string(), "1.000000000000");
    }
    ...
    fn display_subunit_keeps_leading_zeros() {
        ...
        let v = D128s12::from_bits(1_000_000_000);
    ...
    fn lower_exp_fifteen() {
        let v = D128s12::from_bits(15_000_000_000_000);
    ...
    #[test]
    fn lower_exp_zero() {
        assert_eq!(format!("{:e}", D128s12::ZERO), "0e0");
    }
    ...
    fn lower_exp_subunit_negative_exponent() {
        ...
        let v = D128s12::from_bits(1_500_000_000);
    ...
    /// LowerHex of D128s12::ONE is the hex of 10^12 (= 0xe8d4a51000),
    ...
    #[test]
    fn from_str_zero() {
        let v: D128s12 = "0".parse().unwrap();
        assert_eq!(v, D128s12::ZERO);
        let v: D128s12 = "0.0".parse().unwrap();
        assert_eq!(v, D128s12::ZERO);
    }

    #[test]
    fn from_str_one() {
        let v: D128s12 = "1".parse().unwrap();
        assert_eq!(v, D128s12::ONE);
        let v: D128s12 = "1.0".parse().unwrap();
        assert_eq!(v, D128s12::ONE);
    }
    ...
    fn from_str_one_point_one_parses_exactly() {
        let v: D128s12 = "1.1".parse().unwrap();
    ...
    fn from_str_signs() {
        ...
        let pos: D128s12 = "+1.5".parse().unwrap();
    ...
    fn from_str_short_fractional_pads() {
        ...
        let v: D128s12 = "0.5".parse().unwrap();
    ...
    fn from_str_full_scale_fractional() {
        let v: D128s12 = "1.500000000000".parse().unwrap();
    ...
    fn round_trip_representative_values() {
        ...
        for &raw in cases {
            ...
            let s = v.to_string();
            let parsed: D128s12 = s.parse().unwrap_or_else(|e| {
