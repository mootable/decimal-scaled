//! Mathematical constants and float-compatibility constants for [`I128`].
//!
//! # Constants provided
//!
//! The [`DecimalConsts`] trait exposes `pi`, `tau`, `half_pi`,
//! `quarter_pi`, `golden`, and `e` as methods on `I128<SCALE>`.
//!
...
//! practical caller `SCALE`. Each raw constant is the half-away-from-zero
...
//
// Each value is the half-away-from-zero rounding of the canonical decimal
...
// Rescale helper (half-away-from-zero).

/// Rescales `raw` from `SCALE_REF` to `target_scale` using half-away-from-zero
/// rounding.
///
/// - Equal scales: returns `raw` unchanged.
/// - `target_scale < SCALE_REF`: integer division with half-away-from-zero rounding.
...
fn rescale_from_ref(raw: i128, target_scale: u32) -> i128 {
    ...
    if target_scale < SCALE_REF {
        ...
        let half = divisor / 2;
        // Add half with the same sign as `raw` before truncating-toward-zero
        // division to achieve half-away-from-zero semantics.
        let rounded = if raw >= 0 { raw + half } else { raw - half };
        rounded / divisor
    } else {
        // Multiply up; panics in debug on overflow.
...
///
/// Import this trait to call `I128s12::pi()`, `I128s12::e()`, etc.
///
...
pub trait DecimalConsts: Sized {
    ...
    /// `SCALE` with half-away-from-zero rounding.
    ///
    /// # Precision
    ///
    /// N/A: constant value, no arithmetic performed.
    ...
    /// reference rescaled to `SCALE` with half-away-from-zero rounding.
    ///
    /// # Precision
    ///
    /// N/A: constant value, no arithmetic performed.
...
// when `T = I128<SCALE>`. For I128 both equal `I128(1)` -- the smallest
...
mod tests {
    ...
    // 10^23, rounded half-away-from-zero.
    ...
    /// At SCALE = 6, pi() should equal 3.141593 (rounded half-up from
