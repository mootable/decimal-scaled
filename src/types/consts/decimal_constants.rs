...
//! # Sources
//!
//! The constant table is generated by `scripts/gen_const_table.py` from a
//! flint/Arb oracle: each golden mantissa is `floor(const * 10^gp)`
//! resolved by `unique_fmpz`, which yields a value only when the enclosing
//! interval determines every retained digit. Values: pi/tau/pi-2/pi-4
//! (ISO 80000-2), e (OEIS A001113), golden ratio (OEIS A001622), plus
//! log10_2, deg_per_rad (180/pi) and rad_per_deg (pi/180).
