...
//! directly in a `WorkingDecimal` limb buffer and divides via the int layer's
//! width-agnostic `div_rem`. So `dispatch` carries no work-width parameter;
//! it adds only `where Int<N>: WorkingDecimal` for the scratch buffer.
//!
...
/// in the kernel's `WorkingDecimal` scratch buffer, so no work-width type is
/// named here.
#[inline]
#[must_use]
