//! To-radians policy â€” the per-(N, SCALE) algorithm matcher for
//! degrees-to-radians angle conversion.
//!
//! `D<Int<N>, SCALE>::to_radians_strict_with(mode)` delegates directly to
...
fn mul_pi_ratio_routed<const N: usize, const SCALE: u32>(raw: Int<N>, mode: RoundingMode) -> Int<N> {
    match N {
        1 | 2 => crate::algos::trig::trig_series_2limb::to_radians_strict::<SCALE>(
        ...
        _ => crate::algos::trig::trig_series_2limb::to_radians_strict::<SCALE>(
            raw.resize_to::<Int<2>>(), mode).resize_to::<Int<N>>(),
    }
}
...
fn schoolbook_routed<const N: usize, const SCALE: u32>(raw: Int<N>, mode: RoundingMode) -> Int<N> {
    match N {
        1 | 2 => crate::algos::trig::angle_schoolbook::to_radians_schoolbook_narrow::<SCALE>(
            raw.resize_to::<Int<2>>(), mode).resize_to::<Int<N>>(),
        #[cfg(any(feature = "d57", feature = "wide"))]
