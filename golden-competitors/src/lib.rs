...
impl DecimalSubject for F64 {
    ...
    fn mantissa_bits(&self) -> Option<u32> {
        // 53-bit significand: the runner grades against the `2:` hex-float rounded to
        // 53 bits, in binary (spec §2/§3) — re-grading the cells where f64's
        // binary-correct answer differs from the decimal-correct one (~14%).
        Some(53)
    }
...
impl DecimalSubject for F32 {
    ...
    fn mantissa_bits(&self) -> Option<u32> {
        // 24-bit significand: graded against the `2:` hex-float rounded to 24 bits, in
        // binary (spec §2/§3) — re-grades the binary-vs-decimal divergence cells (~7%).
        Some(24)
    }
...
impl DecimalSubject for GMath {
    ...
    fn mantissa_bits(&self) -> Option<u32> {
        // Q128.128 → a 128-bit binary significand. NOTE: the runner's binary grading
        // path implements the f64 (53) / f32 (24) NATIVE widths; the 128-bit compare
        // (a correctly-rounded decimal→128-bit-binary parse of g_math's output) is
        // deferred per the branch's stop-clause (see RADIX_FINDINGS.md), so a `128`
        // cell currently abstains. Declared here so it engages once that path lands.
        Some(128)
    }
