...
fn bench(c: &mut Criterion) {
    for &scale in &[0_usize, 19, 38] {
        ...
        if scale == 19 {
            ...
            g.bench_function("g_math/ln", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("2.0")).ln()))
            });
            g.bench_function("g_math/exp", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).exp()))
            });
            g.bench_function("g_math/sin", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).sin()))
            });
            g.bench_function("g_math/sqrt", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("2.0")).sqrt()))
            });
            g.bench_function("g_math/cos", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).cos()))
            });
            g.bench_function("g_math/tan", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).tan()))
            });
            g.bench_function("g_math/atan", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).atan()))
            });
            g.bench_function("g_math/sinh", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).sinh()))
            });
            g.bench_function("g_math/cosh", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).cosh()))
            });
            g.bench_function("g_math/tanh", |bn| {
                bn.iter(|| evaluate(&black_box(gmath("1.0")).tanh()))
            });
        }

        {
            let a = BigDecimal::new(
                lib_cmp_common::num_bigint_from_two_at_scale(scale),
                scale as i64,
            );
            let b = BigDecimal::new(
                lib_cmp_common::num_bigint_from_one_at_scale(scale),
                scale as i64,
            );
