...
//!
//! Native (`decimal_scaled::Int<4>` / `Int<8>` / `Int<16>`) is compared
...
fn bench_int256(c: &mut Criterion) {
    ...
    four_ops_native_vs_bnum!(g, "Int256", Int<4>, bnum::types::I256);
...
fn bench_int512(c: &mut Criterion) {
    let mut g = c.benchmark_group("wide_int");
    four_ops_native_vs_bnum!(g, "Int512", Int<8>, bnum::types::I512);
    g.finish();
}

fn bench_int1024(c: &mut Criterion) {
    let mut g = c.benchmark_group("wide_int");
    four_ops_native_vs_bnum!(g, "Int1024", Int<16>, bnum::types::I1024);
    g.finish();
}
