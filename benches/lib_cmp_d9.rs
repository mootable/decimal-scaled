...
fn bench(c: &mut Criterion) {
    for &scale in &[0_usize, 5, 9] {
        ...
        match scale {
            0 => {
                let a = D9::<0>::try_from(2).unwrap();
                let b = D9::<0>::try_from(1).unwrap();
                arith_copy!(g, "decimal-scaled", a, b);
            }
            5 => {
                let a = D9::<5>::try_from(2).unwrap();
                let b = D9::<5>::try_from(1).unwrap();
                arith_copy!(g, "decimal-scaled", a, b);
            }
            9 => {
                let a = D9::<9>::try_from(2).unwrap();
                let b = D9::<9>::try_from(1).unwrap();
                arith_copy!(g, "decimal-scaled", a, b);
            }
            _ => unreachable!(),
        }
