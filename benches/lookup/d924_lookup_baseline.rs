...
fn bench_pair<const S_KERNEL: u32, const S_LOOKUP: u32>(c: &mut Criterion, label: &str) {
    let half_k = D924::<S_KERNEL>::from(1) / D924::<S_KERNEL>::from(2);
    let one_p_half_k = D924::<S_KERNEL>::from(1) + half_k;
    let half_l = D924::<S_LOOKUP>::from(1) / D924::<S_LOOKUP>::from(2);
    let one_p_half_l = D924::<S_LOOKUP>::from(1) + half_l;
