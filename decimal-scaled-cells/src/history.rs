...
//!
//! One erased subject per pinned release (the same parsed-handle `Value` shape as
//! the live `DsSubject` in decimal-scale-test — each era's own, since each era's
//! decimal types are its own crate's), generated by [`historical_subject!`] so
...
//!
//! **The parse/format split is symmetry, not decoration.** `tests/history.rs` runs
//! the `Timed` strategy as a ride-along and the history pages publish the result,
//! so the live column and every pinned column must measure the same thing. The live
//! subject times the op alone; a pinned subject that still parsed and formatted
//! inside its closure would hand the current release a fabricated speedup of the
//! parse's size — at the wide tiers, orders of magnitude. Each era therefore gets
//! the identical four-leaf fan-out (parse / compute / format / limits) over its own
//! `CellValue`.
//!
...
macro_rules! historical_subject {
    (
        ...
        pub mod $m {
            ...
            /// Parse one input literal at one concrete decimal type of this version.
            /// The runner's input filter (`limits`) runs first, so a parse failure
            /// here is a golden-data fault; the harness catches the panic.
            ...
            /// One PARSED cell value of THIS release, erased over its width tiers —
            /// the era's counterpart of the live [`crate::CellValue`]. The payload is
            /// the tier's raw storage, carried in that tier's scale-0 alias
            /// (`to_bits`/`from_bits` are a newtype unwrap/wrap, so the carry is
            /// free); the scale rides with the subject.
            ...
            /// The compute leaf of one `(width, scale)` cell of this release: the op
            /// ONLY, over pre-parsed values. Resolved once per execution by
            /// [`dispatch_compute_fn`], never inside a timed span.
            type EraCompute = fn(Function, &[CellValue], HistMode) -> Computed<CellValue>;
            ...
            mod tier_dispatch {
                $(
                    ...
                    pub mod $D {
                        ...
                        /// This tier's scale-erased carrier, in THIS era's crate.
                        type Carrier = $ds::$D<0>;

                        /// Cell type → carrier (a newtype unwrap/wrap; free).
                        #[inline]
                        fn carry<const S: u32>(v: $ds::$D<S>) -> Carrier {
                            <Carrier>::from_bits(v.to_bits())
                        }

                        /// Carrier → cell type at scale `S`; the inverse of [`carry`].
                        #[inline]
                        fn at<const S: u32>(c: Carrier) -> $ds::$D<S> {
                            <$ds::$D<S>>::from_bits(c.to_bits())
                        }

                        /// This tier's carrier out of an erased value. A value of
                        /// another tier cannot occur — the subject parses and computes
                        /// at one cell — so it is a harness fault.
                        #[inline]
                        fn carrier(v: &CellValue) -> Carrier {
                            match v {
                                ...
                                _ => panic!(
                                    "decimal-scaled {} cell value is not a width-{} value",
                                    super::super::VERSION, $w
                                ),
                            }
                        }

                        /// The compute leaf of ONE cell: unwrap, run the op, re-wrap.
                        /// No parse, no format — the whole timed body.
                        fn cell<const S: u32>(
                            func: Function, inputs: &[CellValue], m: HistMode,
                        ) -> Computed<CellValue> {
                            let x = at::<S>(carrier(&inputs[0]));
                            let d2 = inputs.get(1).map(|v| at::<S>(carrier(v)));
                            Computed::Value(CellValue::$D(carry::<S>(
                                super::super::compute(func, x, d2, m),
                            )))
                        }

                        pub fn compute_fn(scale: u32) -> super::super::EraCompute {
                            match scale {
                                $( $s => cell::<{ $s }> as super::super::EraCompute, )+
                                _ => panic!(
                        ...
                        pub fn parse(scale: u32, s: &str) -> CellValue {
                            CellValue::$D(match scale {
                                $( $s => carry::<{ $s }>(super::super::parse_typed::<$ds::$D<$s>>(s)), )+
                                _ => panic!(
                                    "no decimal-scaled {} cell for (width={}, scale={scale})",
                                    super::super::VERSION, $w
                                ),
            ...
            fn dispatch_parse(width: u32, scale: u32, s: &str) -> CellValue {
                match width {
            ...
            fn dispatch_compute_fn(width: u32, scale: u32) -> EraCompute {
                match width {
            ...
            fn dispatch_format(width: u32, scale: u32, v: &CellValue) -> String {
                match width {
            ...
            /// cell tested under one rounding `mode`, `Value = CellValue` (this
            /// era's parsed handle).
            #[derive(Clone, Copy)]
            pub struct Subject {
                pub width: u32,
            ...
            impl DecimalSubject for Subject {
                ...
                fn string_to_value(&self, s: &str) -> CellValue {
                    // The concrete parse, at this era's decimal type for the cell —
                    // hoisted out of `execute` so the timed span holds the op alone,
                    // matching the live subject exactly.
                    dispatch_parse(self.width, self.scale, s)
                }

                fn value_to_string(&self, v: &CellValue) -> String {
                    dispatch_format(self.width, self.scale, v)
                }
                ...
                fn execute(
                    ...
                    let m = hist_mode(mode);
                    // Resolve the cell ONCE, outside every timed span (as the live
                    // subject does), so the `(width, scale)` match is not charged to
                    // the operation.
                    let cell = dispatch_compute_fn(self.width, self.scale);
                    move |inputs| match m {
                        Some(m) => cell(func, inputs, m),
