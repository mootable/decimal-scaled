...
| **0.5.1** (in progress) | **Additive API release.** Operators between decimals of different storage width *and* different `SCALE` — the result takes the wider storage at the **left** operand's scale, so `a + b` has a fixed, predictable type; a declared output type the operator cannot produce is a compile error, never a silently different scale. `requantize` — change storage width and `SCALE` in one call, either direction, ordered so the intermediate cannot overflow a width the target would have held. The scale-only operation renamed `rescale` → `quantize`, the name the decimal arithmetic specification uses (it marks `rescale` as the deprecated spelling); `rescale` / `rescale_with` and `DynDecimal::rescale_to` / `rescale_to_with` remain as deprecated aliases, removed in 0.6.0. Public `expm1` / `log1p`. |
