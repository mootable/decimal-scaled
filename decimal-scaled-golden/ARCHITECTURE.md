...
This document is the design's source of truth. The `DecimalSubject`/`limits`
target it describes is implemented; where the built crate has since extended a
detail beyond what a section records, §11 lists the delta rather than rewriting
the section.
...
- **Fetched to depth + guard, and TRUNCATED — never rounded.** Every oracle
  implements one shared contract (`oracle.format_fetched`): compute
  `floor(|value| · 10^(precision+GUARD))` in its own numeric type, truncating toward
  zero, and hand that integer to the shared formatter. Rounding at a working precision
  can carry across a run of nines and move digits the caller is going to keep;
  flooring cannot. This is not a stylistic preference — half-even at the working
  precision destroyed both guard digits on six `exp` rows, and because the carry also
  manufactures a zero-run it fooled the termination check into calling a
  transcendental value exact at the same time.
- **Format: plain `digits.digits`.** No sentinel, no separator. The two guard
  digits beyond the widest scale have **one job each**, and that is why there are two:
  - the **first guard digit** is the next digit after the widest scale — it decides
    the **round** (below half / half / above half);
  - the **second guard digit** says whether anything survives beneath that — it
    decides **exactness** (a tie is only a true tie if nothing follows).
  ...
  Those two digits are sufficient *because* the value is truncated. A value that has
  been rounded no longer carries an honest first guard digit, which is the whole of
  the defect above: `…4|99` became `…5|00`, and both digits lied in the same stroke.
...
## 11. Implementation notes — where the built crate extends this document

Deltas accumulated since the sections above were written; each is the accurate
current state, recorded here rather than by rewriting the section.

- **`Limits` carries a fourth field** (§3.2): `max_significant_digits:
  Option<u32>` — the total-figure ingestion cap for fixed-*significant*
  libraries (e.g. fastnum D512 ≈ 154, decimal-rs 38). An input whose figure
  span exceeds it is not exactly representable, so the runner skips it — the
  §4.1 exact-input rule extended from fractional depth to total figures.
  `None` = unbounded (arbitrary-precision subjects, and fixed-scale subjects
  whose magnitude envelope already bounds inputs).
- **`DecimalSubject` has a defaulted `name()`** (§3): a diagnostic label
  (capabilities name + config) used e.g. for `ParallelRunner` worker-thread
  names; subjects may override it.
- **§8's terminate-vs-truncate marker is proof-enforced at generation**
  (`oracle/exactness.py`): a value stays stripped (asserting exactness) only
  when exactness is PROVABLE — by theorem for the pure-transcendental family
  (irrational at every in-domain nonzero rational input; exact only at the
  known points), by exact rational inverse-check for the algebraic/power
  family, and by construction for the fraction-generated arithmetic. Anything
...
- **Why `flint`/Arb generates and everything else validates** (§7 states the
  ...
  behind a fixed window and `mpmath` is a point float; neither can pin, and
  `decimal`'s inability to pin is exactly what produced six wrong `exp` rows
  that two oracles then agreed on. `rem` is the one exception, and it is a gap
  rather than a policy: `flint`'s adapter implements no `rem`, so `fraction`
  (exact rationals) generates it.
  Every line is cross-checked by every other available oracle, and each accepted
  line carries a `// generated by .. validated by ..` provenance comment. A
  validator **confirms** when it lands within `ACCEPT_ULPS = 1` unit at the last
  guard digit — one unit, because all oracles floor the same value at the same
  depth, so the only legitimate difference is an internal error straddling the
  truncation boundary. It is annotated `name(delta~MAG)`: the validator's name
  and the size of the difference, and deliberately **no cause**. The earlier form
  appended the radix, and `flint(delta~1e-1233, binary)` sat on wrong rows for
  months because "binary" pre-explained the disagreement as benign and every
  reader skipped it. Whether a delta is acceptable is the tolerance's decision;
  the comment's job is to keep the evidence findable. Beyond the bound the line is
  flagged for a maintainer, never silently dropped.
- **§9 layout, as built**: implemented extras are `loader/filter.rs`
  (`FilterLoader`, the subset decorator), `reporting/inline.rs`
  (`InlineReporter`/`ConsoleReporter` + `RunSummary` — the streaming console
  counterpart of `Reporter`), `execution/criterion_strategy.rs` (§5.2's
  `CriterionStrategy`), and `subject/computed.rs`. The
  `reporting/precision.rs` / `fidelity.rs` / `timing.rs` reporters and
  `validators/binary.rs` listed in §9 are still planned, not yet present.
