...
- **§7's "mpmath = default generator" is superseded by a radix policy**
  (`GENERATOR_POLICY` in `oracle/generate.py`): `fraction` generates the
  arithmetic ops, `decimal` generates sqrt/exp/ln/log10, `flint`/Arb generates
  the remaining irrational transcendentals; mpmath serves as a validator.
  Every line is cross-checked by every other available oracle, and each
  accepted line carries a `// generated by .. validated by ..` provenance
  comment — a bounded binary-vs-base-10 disagreement is annotated
  `name(delta~MAG, radix)`; beyond the bound the line is dropped.
