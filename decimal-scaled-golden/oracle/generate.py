...
def _gen_line(item):
    ...
    if annotations is None:
        return func, None
    prov = f"// generated by: {gen.name()}, validated by: [{', '.join(annotations)}] ; coverage"
...
def cmd_generate(args):
    ...
    # Harvest every function's inputs, then flatten to (func, input, precision) items.
    items = []
    for func in funcs:
        inputs = harvest(func, src)
        if args.limit:
            inputs = inputs[: args.limit]
        items += [(func, inp, args.precision) for inp in inputs]
    total = len(items)
