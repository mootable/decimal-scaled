...
impl Collator {
    ...
    pub fn to_tsv(&self) -> String {
        let mut out = String::from("library\tfunction\twidth\tscale\tmode\toutcome\tdetail\tnanos\n");
        for r in &self.records {
            ...
            out.push_str(&format!(
                ...
                mode_name(r.mode), r.outcome.tag(), detail, nanos,
            ));
        }
        out
    }
...
mod tests {
    ...
    fn rec(lib: &str, f: Function, outcome: Outcome, detail: Option<&str>, nanos: Option<u64>) -> ResultRecord {
        ResultRecord { library: lib.into(), function: f, width: 38, scale: 19,
            mode: RoundingMode::HalfToEven, outcome, detail: detail.map(|s| s.into()), nanos }
    }
    ...
    fn writes_header_and_rows() {
        ...
        assert_eq!(lines[0], "library\tfunction\twidth\tscale\tmode\toutcome\tdetail\tnanos");
