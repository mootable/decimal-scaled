...
graph plots the same medians on a log scale, one line per release, with a shaded
min–max band. Units are shown per cell; the legend maps each to its size in
nanoseconds.

The per-function tables and graphs are split across three category pages:
**[Arithmetic](history/arithmetic.md)**,
**[Roots and Exponents](history/roots-and-exponents.md)**, and
**[Trigonometry](history/trigonometry.md)**.
