...
big set of known-correct answers (the *golden* values — <!-- BEGIN GENERATED:golden:counts -->103,099 answers across 30 functions<!-- END GENERATED:golden:counts -->) and the machinery to check
...
- **Regenerate the functions you touched.** The tool reads the `.pb` inputs, computes
  each answer with `flint`/Arb, **cross-checks it against every other available
  oracle**, and writes `golden/<fn>.au`. A disagreement is never silently kept: within
  one unit at the last guard digit the validator *confirms* and the difference is
  recorded in the line's comment; beyond that the line is flagged for a maintainer to
  investigate. The outcome of an investigation may be to stop trusting one oracle *for
  that value* — it is a judgement, not an automatic filter.

- **How a value is fetched — two rules, one shared implementation.** Every oracle
  obeys the same contract (`oracle.format_fetched`), and none of them formats a value
  itself:

  1. **Fetch to depth + guard, truncated toward zero.** Never rounded. Rounding at a
     working precision can carry across a run of nines and move digits that are going
     to be kept.
  2. **An exact value that terminates shorter than depth + guard keeps its own shorter
     length** — the short fraction *is* the exactness signal. It is honoured only when
     exactness is algebraically PROVABLE (`oracle/exactness.py`); a zero-run on its own
     proves nothing, so an unprovable claim is padded back to the full truncated form.

  Only `flint` generates, because only Arb's rigorous intervals can *pin* a truncation;
  `decimal` and `mpmath` are point values behind a fixed window and can only
  approximate one, which is why they validate. (`rem` is generated by `fraction`, since
  the flint adapter implements no `rem`.)
