//! Foundation surface: `from_bits`/`to_bits`, SCALE/multiplier
//! consts, width smoke tests (moved from `src/types/widths.rs`)
//! and the cross-width/cross-scale `D<S, SCALE>` Eq/Ord semantics
//! (moved from `src/types/unified.rs`). Constructors rewritten to
//! the public `from_bits` + `Int::try_from` / `Int::from` forms.
//! The per-tier `d76`/`d153`/`d307` cfg alternates collapse to the
//! `wide` umbrella (this crate exposes only the umbrella features).
