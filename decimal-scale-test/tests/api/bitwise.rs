//! Bitwise operations on the raw decimal storage (operators, shifts,
//! rotations, counts, and the power-of-two helpers) — storage-not-value
...
mod from_bitwise {
    ...
    fn bitxor_self_is_zero() {
        let a = D38s12::from_bits(decimal_scaled::Int::<2>::try_from(0xDEAD_BEEF_i128).unwrap());
    ...
    fn not_is_self_inverse() {
        let a = D38s12::from_bits(decimal_scaled::Int::<2>::try_from(0xCAFE_i128).unwrap());
        assert_eq!(!!a, a);
    }
    ...
    fn unsigned_shr_zero_fills_negative() {
        ...
        assert_eq!(
            D38s12::from_bits(decimal_scaled::Int::<2>::try_from(-1_i128).unwrap()).unsigned_shr(1),
            D38s12::from_bits(decimal_scaled::Int::<2>::try_from(i128::MAX).unwrap())
        );
    }
    ...
    fn unsigned_shr_zero_amount_identity() {
        let a = D38s12::from_bits(decimal_scaled::Int::<2>::try_from(-42_i128).unwrap());
    ...
    fn rotate_right_low_bit_wraps_to_top() {
        ...
        assert_eq!(
            D38s12::from_bits(decimal_scaled::Int::<2>::try_from(1_i128).unwrap()).rotate_right(1),
            D38s12::from_bits(decimal_scaled::Int::<2>::try_from(i128::MIN).unwrap())
        );
    }
    ...
    fn rotate_right_round_trip() {
        let a = D38s12::from_bits(decimal_scaled::Int::<2>::try_from(0xCAFE_F00D_i128).unwrap());
...
mod from_macros_bitwise_and_overflow {
    //! The bit-manipulation blocks of the retired
    //! `tests/macros_bitwise_and_overflow.rs`; its overflow and float-bridge
    //! blocks live in `contracts/overflow.rs` and `api/conversions.rs`.
...
mod from_macros_surface {
    //! The bitwise-operator block of the retired `tests/macros_surface.rs`.
