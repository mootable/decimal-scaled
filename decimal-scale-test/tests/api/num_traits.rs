//! Saturating `from_num` / `to_num` (`NumCast`-bridge) behaviour
//! tests moved from `src/types/num_traits.rs`. Kept in full: the
//! `tests/num_traits.rs` + `macros_num_traits.rs` suites cover the
//! `num_traits` TRAIT impls but not this inherent saturating pair.
