...
//! free functions. Requires the `cross-scale-ops` feature (declared as this
//! target's `required-features`, so a build without it skips the target) and
//! the nightly toolchain (`generic_const_exprs`).
//! Migrated from `tests/cross_scale_nightly.rs`.
