//! Routing / dispatcher-wiring contracts: the plain `*` dispatcher resolves
//! to `*_strict`, the `_with(DEFAULT)` siblings match their no-arg forms, and
//! the AGM alternates agree with the canonical kernels.
//! Migrated from `tests/wide_roots_dispatcher_and_hypot.rs` plus the
//! dispatcher / `_with`-sibling / AGM contract halves of
//! `tests/narrow_strict_transcendentals.rs` and
//! `tests/wide_strict_transcendentals.rs` (whose value cross-checks stay put
//! until the golden-lead fold retires them).
