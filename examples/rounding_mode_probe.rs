...
fn main() {
    ...
    // e = 2.71828...
    let e_hi = Hi::try_from(1_i64).unwrap().exp();
    print_candidates("exp(1) = e", e_hi);

    // sin(1)
    let sin1_hi = Hi::try_from(1_i64).unwrap().sin();
    print_candidates("sin(1)", sin1_hi);

    // ln(2)
    let ln2_hi = Hi::try_from(2_i64).unwrap().ln();
    print_candidates("ln(2)", ln2_hi);

    // sqrt(2)
    let sqrt2_hi = Hi::try_from(2_i64).unwrap().sqrt();
    ...
    println!("  exp(1)   = {}", D38::<19>::try_from(1_i64).unwrap().exp());
    println!("  sin(1)   = {}", D38::<19>::try_from(1_i64).unwrap().sin());
    println!("  ln(2)    = {}", D38::<19>::try_from(2_i64).unwrap().ln());
    println!("  sqrt(2)  = {}", D38::<19>::try_from(2_i64).unwrap().sqrt());
}
