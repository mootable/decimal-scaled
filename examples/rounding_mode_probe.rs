...
fn main() {
    ...
    // e = 2.71828...
    let e_hi = Hi::from_int(1).exp_strict();
    print_candidates("exp(1) = e", e_hi);

    // sin(1)
    let sin1_hi = Hi::from_int(1).sin_strict();
    print_candidates("sin(1)", sin1_hi);

    // ln(2)
    let ln2_hi = Hi::from_int(2).ln_strict();
    print_candidates("ln(2)", ln2_hi);

    // sqrt(2)
    let sqrt2_hi = Hi::from_int(2).sqrt_strict();
    ...
    println!("  exp(1)   = {}", D38::<19>::from_int(1).exp_strict());
    println!("  sin(1)   = {}", D38::<19>::from_int(1).sin_strict());
    println!("  ln(2)    = {}", D38::<19>::from_int(2).ln_strict());
    println!("  sqrt(2)  = {}", D38::<19>::from_int(2).sqrt_strict());
}
